...
一个**浏览器端**的 Claude Code 本地数据(`~/.claude`)可视化与管理界面,参考 OpenAI Codex 客户端的开发者工具风格(Codex/Linear 气质,可切换深/浅色)。可浏览会话/转写(含工具调用与 diff)、管理 MCP / Skills / Memory / Settings / Plans(含项目级 permissions/hooks)、跨数据源全局搜索、`Ctrl/Cmd+K` 命令面板,以及一键 `claude --resume` 接回某个项目。
...
- `Ctrl/Cmd+K` — 命令面板(导航 9 个页面、切换主题/语言、跳转全局搜索)
- `Ctrl/Cmd+J` — 切换深/浅色主题
- `Ctrl/Cmd+Shift+L` — 切换 中文 / English
- `/` — 跳转全局搜索
- 输入框聚焦时单字符快捷键自动禁用
...
- **主题**:CSS 自定义属性 token(`src/styles/theme-tokens.css`)同时供 Tailwind 与 Naive UI `themeOverrides`(`src/theme.ts`)消费;`<html data-theme>` 切换深/浅色,由 `useTheme`(`src/composables/useTheme.ts`)驱动(localStorage + `prefers-color-scheme`)。
...
- **v1.4(完成)**:① 性能——`highlight.js` 改按需加载(常用语言子集,`MarkdownBlock` chunk ~1MB→~206KB)+ `summarize()` 加 mtime 失效缓存;② 跨平台 resume——macOS(osascript)/Linux(自动探测终端)开终端跑 `claude --resume`;③ 明暗主题切换(浅色 token 已就绪,补 `useTheme` + Naive light overrides + 侧栏开关 + 统一 `--code-bg`);④ `Ctrl/Cmd+K` 命令面板 + 全局快捷键;⑤ 项目级 `.claude/settings.json` 编辑器(permissions allow/deny/ask + defaultMode + hooks + env + model)。另修复 `encodeCwd` 漏替换 `/`(正斜杠项目路径现已正确编码)。
...
- **接回 Claude(完成)**:一键 **resume** —— 在项目 cwd 开终端跑 `claude --resume <id>`。**跨平台**:Windows / macOS(Terminal)/ Linux(自动探测终端)。
- **全局搜索(完成)**:`/search` 跨会话 + 记忆 + 技能 + 计划,命中高亮片段、点击跳转。
- **i18n(完成)**:自带 中文 / English 开关。
- 生产构建已验证(`npm run build` → `dist/`,由 `npm start` 托管)。

### 后续可做
- 转写列表虚拟化(超长会话,目前是分页 load-more;转写块高度可变,需动态测量)。

> v1.4 已交付:`highlight.js` 按需加载、`summarize()` 缓存、项目级 settings 编辑器、跨平台 resume、明暗主题、命令面板。

## 平台说明

- **resume**("继续会话" / 开终端跑 `claude --resume`)**跨平台**:Windows / macOS(Terminal,经 osascript)/ Linux(优先 `$TERMINAL`,否则依次探测 gnome-terminal / konsole / xfce4-terminal / lxterminal / alacritty / kitty;全部缺失时返回 HTTP 501)。
